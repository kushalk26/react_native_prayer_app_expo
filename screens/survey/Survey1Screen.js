...
function Survey1Screen({ navigation }) {
  ...
  const handleOptionChange = (option) => {
	...
	const addSurvey = async(item_survey,checked) => {
		setIsLoading(true);
		const sessionData = await AsyncStorage.getItem('sessionData');	
		if (sessionData !== null) {
			const parsedSessionData = JSON.parse(sessionData);
	...
	const handleItemPress = async(item,survey_i) => {
			  ...
			  {total_data > 0 ? (
				value === 'text' ? (
					<View>
						...
						<TouchableOpacity onPress={() => handleItemPress(item,survey_item)}>
