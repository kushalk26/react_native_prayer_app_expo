...
function NotificationScreen({ navigation }) {
	...
	const fetchData = async() => { 
		...
		if (sessionData !== null) {
			...
			if(response.data){								
				...
				console.log("notification data: ", res_data);
	...
	const handlePress = async(item) => {
		console.log(item);
		navigation.navigate('SingleNotification', { item });	
	};
 
	const determineStyle = (condition) => {
	  if (condition.status==0) {
		return styles.NotificationBackground;
	  }
	  return styles.NotificationBackgroundGray;
	};
				...
				{isLoading ? (
					...
					data.map(item_data => (
						<View key={item_data.id} style={styles.notification} >
							<View key={item_data.id} style={[styles.container, determineStyle(item_data)]}>
								<TouchableOpacity onPress={() => handlePress(item_data)}>
									<View style={styles.notificatonStyle}>
										<Text style={styles.notificationText}><First100Words testName={item_data.notification_name} /></Text>
									</View>
									<View style={styles.notificatonStyle}>
										<Text style={styles.notificationDate}><DateTimeComponent dateString={item_data.created_at} /></Text>
									</View>
